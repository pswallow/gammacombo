...
Double_t RooIpatia2__low_x_LnBK(Double_t nu, Double_t x) {
  return TMath::Log(TMath::Gamma(nu)) + (nu - 1.) * RooIpatia2__log_de_2 -
         nu * TMath::Log(x);
}
...
Double_t RooIpatia2__BK(Double_t ni, Double_t x) {
  ...
  if (x < 1.e-06 && nu > 0.)
    return RooIpatia2__low_x_BK(nu, x);
...
Double_t RooIpatia2__LnBK(double ni, double x) {
  Double_t nu = TMath::Abs(ni);
  if (x < 1.e-06 && nu > 0.)
    return RooIpatia2__low_x_LnBK(nu, x);
  if (x < 1.e-04 && nu > 0. && nu < 55.)
    return RooIpatia2__low_x_LnBK(nu, x);
  if (x < 0.1 && nu >= 55.)
    return RooIpatia2__low_x_LnBK(nu, x);
...
Double_t RooIpatia2__LogEval(Double_t d, Double_t l, Double_t alpha,
                             Double_t beta, Double_t delta) {
  // Double_t d = x-mu;
  ...
  Double_t logno = l * TMath::Log(gamma / delta) - RooIpatia2__logsq2pi -
                   RooIpatia2__LnBK(l, dg);
  ...
  return TMath::Exp(
      ...
      RooIpatia2__LnBK(
          l - 0.5,
          alpha *
              TMath::Sqrt(thing))); // + TMath::Log(TMath::Abs(beta)+0.0001) );
}

Double_t RooIpatia2__diff_eval(Double_t d, Double_t l, Double_t alpha,
                               Double_t beta, Double_t delta) {
  // Double_t RooIpatia2__sq2pi = TMath::Sqrt(2*TMath::ACos(-1));
  ...
  Double_t dg = delta * gamma;
  // Double_t mu_ = mu;// -
  // delta*beta*RooIpatia2__BK(l+1,dg)/(gamma*RooIpatia2__BK(l,dg)); Double_t d =
  // x-mu;
  Double_t thing = delta * delta + d * d;
  Double_t sqthing = TMath::Sqrt(thing);
  Double_t alphasq = alpha * sqthing;
  Double_t no = TMath::Power(gamma / delta, l) / RooIpatia2__BK(l, dg) *
                RooIpatia2__sq2pi_inv;
  Double_t ns1 = 0.5 - l;

  return no * TMath::Power(alpha, ns1) * TMath::Power(thing, l / 2. - 1.25) *
         (-d * alphasq *
              (RooIpatia2__BK(l - 1.5, alphasq) +
               RooIpatia2__BK(l + 0.5, alphasq)) +
          (2. * (beta * thing + d * l) - d) * RooIpatia2__BK(ns1, alphasq)) *
         TMath::Exp(beta * d) / 2.;
}
...
ClassImp(RooIpatia2)
    ...
    : RooAbsPdf(name, title), x("x", "x", this, _x), l("l", "l", this, _l),
      zeta("zeta", "zeta", this, _zeta), fb("fb", "fb", this, _fb),
      sigma("sigma", "sigma", this, _sigma), mu("mu", "mu", this, _mu),
      a("a", "a", this, _a), n("n", "n", this, _n), a2("a2", "a2", this, _a2),
      n2("n2", "n2", this, _n2) {}

RooIpatia2::RooIpatia2(const RooIpatia2 &other, const char *name)
    : RooAbsPdf(other, name), x("x", this, other.x), l("l", this, other.l),
      zeta("zeta", this, other.zeta), fb("fb", this, other.fb),
      sigma("sigma", this, other.sigma), mu("mu", this, other.mu),
      a("a", this, other.a), n("n", this, other.n), a2("a2", this, other.a2),
      n2("n2", this, other.n2) {}
...
Double_t RooIpatia2::evaluate() const {
  ...
  Double_t out = 0.;
  if (zeta != 0.) {
    phi = RooIpatia2__BK(l + 1., zeta) /
          RooIpatia2__BK(
              l, zeta); // careful if zeta -> 0. You can implement a function
                        // for the ratio, but carefull again that |nu + 1 | !=
                        // |nu| + 1 so you jave to deal wiht the signs
    ...
    if (l <= -1.0) {
      ...
      printf("WARNING: zeta ==0 and l > -1 ==> not defined rms. Changing the "
    ...
    if (d < -asigma) {
      ...
      k1 = cons1 * TMath::Power(phi, l - 0.5);
      k2 = beta * k1 + cons1 * (l - 0.5) * TMath::Power(phi, l - 1.5) * 2. *
                           a2sigma / delta2;
      B = -a2sigma - n2 * k1 / k2;
      A = k1 * TMath::Power(B + a2sigma, n2);
      out = A * TMath::Power(B + d, -n2);
...
Double_t RooIpatia2::analyticalIntegral(Int_t code, const char* rangeName) const
{
    ...
    if (code==1) {
        ...
        if ((d0 > -asigma) && (d1 < a2sigma)){ return  stIntegral(d1,delta, l) -
stIntegral(d0,delta, l);}
        ...
        if (d0 > a2sigma) {
            ...
            k1 = cons1*TMath::Power(phi,l-0.5);
            k2 = beta*k1+
cons1*(l-0.5)*TMath::Power(phi,l-1.5)*2.*a2sigma/delta2; B = -a2sigma -
n2*k1/k2; A = k1*TMath::Power(B+a2sigma,n2); return
A*(TMath::Power(B+d1,1-n2)/(1-n2) -TMath::Power(B+d0,1-n2)/(1-n2) ) ;
        }
        ...
        if (d0 <-asigma) {
            ...
            A = k1*TMath::Power(B+asigma,n);
            I0 = A*TMath::Power(B-d0,1-n)/(n-1);
            I1a = A*TMath::Power(B+asigma,1-n)/(n-1) - stIntegral(-asigma,delta,
l);
        }
        ...
        if (d1 > a2sigma) {
            ...
            k2 = beta*k1+
cons1*(l-0.5)*TMath::Power(phi,l-1.5)*2.*a2sigma/delta2; B = -a2sigma -
n2*k1/k2; A = k1*TMath::Power(B+a2sigma,n2); I1b =
A*(TMath::Power(B+d1,1-n2)/(1-n2) -TMath::Power(B+a2sigma,1-n2)/(1-n2) ) -
